...
void main()
{
	...
	// Light attenuation
	float d_l = length(light - fragment.position);
	float alpha_l = min(d_l / attenuation_distance, 1.0);
	vec3 Cl = (1.0 - alpha_l) * light_color;

	float d_max = fog_distance;
	float d = length(camera_position - fragment.position);
	float alpha_f = min(d / d_max, 1.0);
	
	color_shading = (1.0 - alpha_f) * color_shading * Cl + alpha_f*fog_color;
